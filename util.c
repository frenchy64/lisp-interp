 ...
 *
 * @return non-zero if c is alphabetic, otherwise zero
 **/
int is_alpha(int c) {
  return isalpha(c);
}
 ...
 *
 * @return non-zero if c is alphanumeric, otherwise zero
 **/
...
/**
 * Determine if character is either ' ', '\f', '\n', '\r',
 * '\t' or '\v'.
 *
 * @return non-zero if c is whitespace, otherwise zero
 **/
